...
ERROR_T AiIdentDataRun(void) {
	...
	AiBaseUpdateDlgStatus("%s", "Convert to ai net image cmodel2.");
	result = AiNetImageCmodel2(&(g_ai_ident_data_rw.image_net_cmodel2), &(g_ai_ident_data_rw.image_1080p));
	if (result != AI_ERROR_SUCCESS) {
		return AI_ERROR_GENERAL;
	}

	AiBaseUpdateDlgStatus("%s", "Ident the net image.");
	result = AiIdentPredict(&(g_ai_ident_data_rw.net_state), AiNetDataGetAiNetNetwork(), AiNetDataGetAiNetLayer(), g_ai_ident_data_rw.image_net_cmodel2.data);
	if (result != AI_ERROR_SUCCESS) {
		return AI_ERROR_GENERAL;
	}
