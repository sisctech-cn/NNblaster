...
ERROR_T AiBaseLnetModbusSlaveRxHeadTail(AI_BASE_LNET_LINK_PTR link, AI_BASE_LNET_PACKET_PTR packet)
{
    ...
    while (1) {
        if (AiBaseLnetModbusDataGet(packet, start, AI_BASE_LNET_PACKET_DEST_ADDR_LENGTH, &dest_address) != AI_ERROR_SUCCESS) {
	...
	if (function_code == AI_BASE_LNET_PACKET_FCODE_WRITE_HOLD_REGS) {
        ...
        if (AiBaseLnetModbusDataGet(packet, start+AI_BASE_LNET_PACKET_DEST_ADDR_LENGTH+AI_BASE_LNET_PACKET_FCODE_LENGTH+AI_BASE_LNET_PACKET_REG_START_ADDR_LENGTH+AI_BASE_LNET_PACKET_REG_NUMBER_LENGTH,
                AI_BASE_LNET_PACKET_LENGTH_LENGTH, &packet_length) != AI_ERROR_SUCCESS) {
            return AI_ERROR_GENERAL;
        }
        packet_length = AI_BASE_LNET_PACKET_DEST_ADDR_LENGTH+AI_BASE_LNET_PACKET_FCODE_LENGTH+AI_BASE_LNET_PACKET_REG_START_ADDR_LENGTH+AI_BASE_LNET_PACKET_REG_NUMBER_LENGTH+AI_BASE_LNET_PACKET_LENGTH_LENGTH+packet_length;
	}
...
ERROR_T AiBaseLnetModbusSlaveDecode(AI_BASE_LNET_PACKET_PTR packet, AI_BASE_LNET_PACKET_DECODE_PTR decode)
{
    ...
    /* get source address */
    decode->source_addr = (UINT8_T)0;

    /* get dest address */
    if (AiBaseLnetModbusDataGet(packet, start, AI_BASE_LNET_PACKET_DEST_ADDR_LENGTH, &data) != AI_ERROR_SUCCESS) {
        return AI_ERROR_GENERAL;
    }
	...
	if (decode->command == AI_BASE_LNET_PACKET_FCODE_WRITE_HOLD_REGS) {
        ...
        decode->packet_public.reg_start_address = reg_start_address;
		...
		start = start + AI_BASE_LNET_PACKET_REG_NUMBER_LENGTH;
	    ...
	    decode->packet_public.reg_number = reg_number;
	}
	/* write hold reg */
    ...
    for (field_index = 0; field_index < AI_BASE_LNET_PACKET_FIELD_MAX; field_index++) {
        ...
        decode->field[field_index].index = (decode->packet_public.reg_start_address + field_index);
...
ERROR_T AiBaseLnetModbusSlaveCommand(AI_BASE_LNET_PACKET_PTR packet, AI_BASE_LNET_PACKET_DECODE_PTR decode, UINT32_T inteface)
{
	...
	case AI_BASE_LNET_PACKET_FCODE_READ_INPUT_REGS:
        if (flag == AI_BASE_LNET_PACKET_COMMAND_FLAG_RECV) {
			if (retcode == AI_BASE_LNET_PACKET_COMMAND_RETCODE_SUCCESS) {
				result = AiBaseLnetModbusCommandRead(packet, decode, inteface);
			} else {
	...
	case AI_BASE_LNET_PACKET_FCODE_WRITE_HOLD_REGS:
        if (flag == AI_BASE_LNET_PACKET_COMMAND_FLAG_RECV) {
            if (retcode == AI_BASE_LNET_PACKET_COMMAND_RETCODE_SUCCESS) {
				result = AiBaseLnetModbusCommandWrite(packet, decode, inteface);
			} else {
				result = AI_ERROR_GENERAL;
			}
        }
...
ERROR_T AiBaseLnetModbusSlaveEncode(AI_BASE_LNET_PACKET_DECODE_PTR decode, ERROR_T retcode, AI_BASE_LNET_PACKET_PTR packet)
{
    ...
    /* set dest address */
    if (AiBaseLnetModbusDataSet(packet, end, AI_BASE_LNET_PACKET_DEST_ADDR_LENGTH, decode->dest_addr) != AI_ERROR_SUCCESS) {
        return AI_ERROR_GENERAL;
    }
	...
	if (decode->from == AI_BASE_LNET_PACKET_COMMAND_FLAG_SEND) {
		...
		if ((decode->command == AI_BASE_LNET_PACKET_FCODE_WRITE_HOLD_REG) ||
		    (decode->command == AI_BASE_LNET_PACKET_FCODE_READ_HOLD_REGS) ||
		    (decode->command == AI_BASE_LNET_PACKET_FCODE_READ_INPUT_REGS)) {
			for (field_index = 0; field_index < AI_BASE_LNET_PACKET_FIELD_MAX; field_index++) {
