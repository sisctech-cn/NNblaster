...
INT32_T AiBaseFloat64ClzU16(UINT16_T a)
{
    ...
    if (b == 0) {
        ...
        r = AiBaseFloat64BuiltinClz(b) - 16;
    }
    return r;
}
...
INT32_T AiBaseFloat64ClzU64(UINT64_T a)
{
    ...
    if (a == 0) {
		...
		if ((a >> 32) > 0) {
			r = AiBaseFloat64BuiltinClz((UINT32_T)(a >> 32));
		} else {
			r = 32 + AiBaseFloat64BuiltinClz((UINT32_T)(a & 0x00000000FFFFFFFF));
		}
    }
    return r;
}
...
INT32_T AiBaseFloat64CtzU16(UINT16_T a)
{
    ...
    if (b == 0) {
        ...
        r = AiBaseFloat64BuiltinCtz(b) - 16;
    }
    return r;
}
...
INT32_T AiBaseFloat64CtzU64(UINT64_T a)
{
    ...
    if (a == 0) {
		...
		if ((a & 0x00000000FFFFFFFF) > 0) {
			r = AiBaseFloat64BuiltinCtz((UINT32_T)(a & 0x00000000FFFFFFFF));
		} else {
			r = 32 + AiBaseFloat64BuiltinCtz((UINT32_T)(a >> 32));
		}
    }
    return r;
}
...
INT8_T *AiBaseFloat64Pstrcat(INT8_T *buf, INT32_T buf_size, const INT8_T *s)
{
    ...
    if (len < buf_size)
        AiBaseFloat64Pstrcpy(buf + len, buf_size - len, s);
    return buf;
}
...
{
    AiBaseFloat64DbufInit2(s, NULL, AiBaseFloat64DbufDefaultRealloc);
}
...
INT32_T AiBaseFloat64DbufWrite(AI_BASE_FLOAT64_DYN_BUF *s, UINT32_T offset, const UINT8_T *data, UINT32_T len)
{
    ...
    if (AiBaseFloat64DbufRealloc(s, end))
        return -1;
...
INT32_T AiBaseFloat64DbufPut(AI_BASE_FLOAT64_DYN_BUF *s, const UINT8_T *data, UINT32_T len)
{
    if (((s->size + len) > s->allocated_size)) {
        if (AiBaseFloat64DbufRealloc(s, s->size + len))
            return -1;
    }
...
{
    return AiBaseFloat64DbufPut(s, &c, 1);
}
...
{
    return AiBaseFloat64DbufPut(s, (const UINT8_T *)str, (UINT32_T)strlen(str));
}
...
INT32_T AiBaseFloat64DbufPrintf(AI_BASE_FLOAT64_DYN_BUF *s, const INT8_T *fmt, ...)
{
    ...
    if (len < sizeof(buf)) {
        /* fast case */
        return AiBaseFloat64DbufPut(s, (UINT8_T *)buf, len);
    } else {
        if (AiBaseFloat64DbufRealloc(s, s->size + len + 1))
            return -1;
        va_start(ap, fmt);
...
{
    /* we test s->buf as a fail safe to avoid crashing if AiBaseFloat64DbufFree()
       is called twice */
    if (s->buf) {
