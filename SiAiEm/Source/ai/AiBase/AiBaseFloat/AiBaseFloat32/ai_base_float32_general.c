...
INT32_T AiBaseFloat32ClzU16(UINT16_T a)
{
    ...
    if (b == 0) {
        ...
        r = AiBaseFloat32BuiltinClz(b) - 16;
    }
    return r;
}
...
INT32_T AiBaseFloat32ClzU64(UINT64_T a)
{
    ...
    if (a == 0) {
		...
		if ((a >> 32) > 0) {
			r = AiBaseFloat32BuiltinClz((UINT32_T)(a >> 32));
		} else {
			r = 32 + AiBaseFloat32BuiltinClz((UINT32_T)(a & 0x00000000FFFFFFFF));
		}
    }
    return r;
}
...
INT32_T AiBaseFloat32CtzU16(UINT16_T a)
{
    ...
    if (b == 0) {
        ...
        r = AiBaseFloat32BuiltinCtz(b) - 16;
    }
    return r;
}
...
INT32_T AiBaseFloat32CtzU64(UINT64_T a)
{
    ...
    if (a == 0) {
		...
		if ((a & 0x00000000FFFFFFFF) > 0) {
			r = AiBaseFloat32BuiltinCtz((UINT32_T)(a & 0x00000000FFFFFFFF));
		} else {
			r = 32 + AiBaseFloat32BuiltinCtz((UINT32_T)(a >> 32));
		}
    }
    return r;
}
...
INT8_T *AiBaseFloat32Pstrcat(INT8_T *buf, INT32_T buf_size, const INT8_T *s)
{
    ...
    if (len < buf_size)
        AiBaseFloat32Pstrcpy(buf + len, buf_size - len, s);
    return buf;
}
...
{
    AiBaseFloat32DbufInit2(s, NULL, AiBaseFloat32DbufDefaultRealloc);
}
...
INT32_T AiBaseFloat32DbufWrite(AI_BASE_FLOAT32_DYN_BUF *s, UINT32_T offset, const UINT8_T *data, UINT32_T len)
{
    ...
    if (AiBaseFloat32DbufRealloc(s, end))
        return -1;
...
INT32_T AiBaseFloat32DbufPut(AI_BASE_FLOAT32_DYN_BUF *s, const UINT8_T *data, UINT32_T len)
{
    if (((s->size + len) > s->allocated_size)) {
        if (AiBaseFloat32DbufRealloc(s, s->size + len))
            return -1;
    }
...
{
    return AiBaseFloat32DbufPut(s, &c, 1);
}
...
{
    return AiBaseFloat32DbufPut(s, (const UINT8_T *)str, (UINT32_T)strlen(str));
}
...
INT32_T AiBaseFloat32DbufPrintf(AI_BASE_FLOAT32_DYN_BUF *s, const INT8_T *fmt, ...)
{
    ...
    if (len < sizeof(buf)) {
        /* fast case */
        return AiBaseFloat32DbufPut(s, (UINT8_T *)buf, len);
    } else {
        if (AiBaseFloat32DbufRealloc(s, s->size + len + 1))
            return -1;
        va_start(ap, fmt);
...
{
    /* we test s->buf as a fail safe to avoid crashing if AiBaseFloat32DbufFree()
       is called twice */
    if (s->buf) {
