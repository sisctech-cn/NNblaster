...
void AiNetCmodel2LayerCommandGenerate(INT32_T layer_index, UINT64_T net_state_address)
{
	...
	if (layer_data_next != NULL) {
		AiBaseCmodel2CommandLayerInit(
			...
			layer_data->h, 
			layer_data->c, 
			layer_data->layer_config->size.value,
			layer_data->layer_config->stride.value,
			layer_data->layer_config->filters.value,
			layer_data_next->layer_config->size.value);
	}
	else {
