/**
 ******************************************************************************
 * file    ai_net_image_cmodel1_ir.c
 * author  sunlingge
 * version V100
 * brief   the ai net image tna_ir driver.
 ******************************************************************************
 */
/*------------------------- Include File ------------------------------------*/
 ...
 * comment  V100
 */
ERROR_T AiNetImageCmodel1Save(AI_NET_IMAGE_CMODEL1 *image_net)
{
    UINT8_T file_path[256];
	ERROR_T result;

    sprintf((char *)file_path, AI_PRODUCT_LOG_IDENT_PATH"/netCmodel1.png");
...
ERROR_T AiNetImageCmodel1(AI_NET_IMAGE_CMODEL1 *dest, AI_BASE_IMAGE_1080P *source)
{
	...
	AiBaseImageMakeImage(AI_NET_IMAGE_CMODEL1_WIDTH, AI_NET_IMAGE_CMODEL1_HEIGHT, AI_NET_IMAGE_CMODEL1_CHANNEL, (AI_BASE_IMAGE *)dest, AI_NET_IMAGE_CMODEL1_CONTENT_LENGTH_MAX);

	content = (FLOAT_AI_T *)g_ai_base_image_1080p_content;
	part_w = AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING*2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK;
	part_h = source->h;
	part_c = source->c;
	w_scale = (FLOAT_AI_T)AiBaseFloatDiv(AiBaseFloatCvtF32Fai((FLOAT32_T)(source->w - 1)),
				AiBaseFloatCvtF32Fai((FLOAT32_T)(AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING*2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK - 1)));
	h_scale = (FLOAT_AI_T)AiBaseFloatDiv(AiBaseFloatCvtF32Fai((FLOAT32_T)(source->h - 1)), 
				AiBaseFloatCvtF32Fai((FLOAT32_T)(AI_NET_IMAGE_CMODEL1_HEIGHT - 1)));
	for (k = 0; k < source->c; k++) {
		for (r = 0; r < source->h; ++r) {
			for (c = 0; c < AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING * 2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK; ++c) {
				val = 0;
				if ((c == AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING * 2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK - 1) || (source->w == 1)) {
	...
	for (k = 0; k < source->c; ++k) {
		for (r = 0; r < AI_NET_IMAGE_CMODEL1_HEIGHT; ++r) {
			...
			for (c = 0; c < AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING * 2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK; ++c) {
				...
				AiBaseImageSetPixel(dest->data, dest->w, dest->h, dest->c, c+ AI_NET_IMAGE_CMODEL1_WIDTH_PADDING, r, k, val);
			}
			if ((r == AI_NET_IMAGE_CMODEL1_HEIGHT - 1) || (source->h == 1))
				continue;
			for (c = 0; c < AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING * 2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK; ++c) {
				val = AiBaseFloatMul(dy, AiBaseImageGetPixel(content, part_w, part_h, part_c, c, iy + 1, k));
				AiBaseImageAddPixel(dest->data, dest->w, dest->h, dest->c, c + AI_NET_IMAGE_CMODEL1_WIDTH_PADDING, r, k, val);
			}
		}
	}
	...
	for (r = 0; r < AI_NET_IMAGE_CMODEL1_HEIGHT; r++) {
		for (k = 0; k < dest->c; ++k) {
			...
			for (c = 0; c < AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK; ++c) {
				AiBaseImageAddPixel(dest->data, dest->w, dest->h, dest->c,
					AI_NET_IMAGE_CMODEL1_WIDTH - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING * 2 - AI_NET_IMAGE_CMODEL1_WIDTH_PADDING_ALIGNED_MBLOCK + c, r, k, val);
			}
		}
	}
...
#endif
#if 1
	AiBaseLogLayerData(0, "NetPngCmodel1", (char *)(dest->data), (dest->data_len * sizeof(FLOAT_AI_T)));
#endif
	return AI_ERROR_SUCCESS;
}
