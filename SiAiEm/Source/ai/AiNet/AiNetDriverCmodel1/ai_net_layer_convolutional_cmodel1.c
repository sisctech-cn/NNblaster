...
/*------------------------- Variable Definition------------------------------*/
FLOAT_AI_T g_ai_cmodel1_map_padding_input[AI_NET_NETWORK_BUFFER_LENGTH_MAX];
FLOAT_AI_T g_ai_cmodel1_map_padding_output[AI_NET_NETWORK_BUFFER_LENGTH_MAX];
...
ERROR_T AiNetLayerConvolutionalForward(AI_NET_STATE *net_state, UINT32_T layer_index) 
{
	...
	batch_output = output;
	for (i = 0; i < batch; i++){
#if (AI_PRODUCT_OPTION_CMODEL1_GEMM_TYPE == AI_PRODUCT_CMODEL1_GEMM_TYPE_CPU)
		AiNetLogCmodel1LayerWeights(layer_index, "WeightsCmodel1", (UINT8_T *)(weights), layer_data->weights.length);
		AiNetLogCmodel1LayerInput(layer_index, "InputCmodel1", (UINT8_T *)(net_state->input), (inputs_align * sizeof(FLOAT_AI_T)));
		AiBaseBoardCmodel1AioMapCopyCpu(batch, w_align, h_align, c_align, padding, batch_input, w, h, c, 0, g_ai_cmodel1_map_padding_input);
		AiNetLogCmodel1LayerInput(layer_index, "im2colInputCmodel1", (UINT8_T *)(g_ai_cmodel1_map_padding_input), (c*h*w * sizeof(FLOAT_AI_T)));
		AiBaseBoardX86AioIm2colCpuCustom(g_ai_cmodel1_map_padding_input, c, h, w, size, stride, padding, workspace);
		AiBaseLogLayerData(layer_index, "im2colOutputCmodel1", (char *)(workspace),
			(c*size*size * ((h + 2 * padding - size) / stride + 1) * ((w + 2 * padding - size) / stride + 1) * sizeof(FLOAT_AI_T)));
		memset((char *)g_ai_cmodel1_map_padding_output, 0, AI_NET_NETWORK_BUFFER_LENGTH_MAX*sizeof(FLOAT_AI_T));
		AiBaseBoardX86AioGemm(0, 0, m, n, k, AiBaseFloatCvtF32Fai(1.0f), weights, k, workspace, n, AiBaseFloatCvtF32Fai(1.0f), g_ai_cmodel1_map_padding_output, n);
		AiBaseBoardCmodel1AioMapCopyCpu(batch, out_w, out_h, out_c, 0, g_ai_cmodel1_map_padding_output, out_w_align, out_h_align, out_c_align, padding_next, batch_output);
		AiNetLogCmodel1LayerOutput(layer_index, "GemmOutputCmodel1", (UINT8_T *)(batch_output), (outputs_align * sizeof(FLOAT_AI_T)));
#endif
#if (AI_PRODUCT_OPTION_CMODEL1_GEMM_TYPE == AI_RRODUCT_CMODEL1_GEMM_TYPE_CMODEL1)
		if (layer_index == 0) {
		...
		AiNetCmodel1LayerCommandGenerate(layer_index, (UINT64_T)net_state);
		AiNetLogCmodel1LayerWeights(layer_index, "WeightsCmodel1", (UINT8_T *)(weights), layer_data->weights.length);
		AiNetLogCmodel1LayerInput(layer_index, "InputCmodel1", (UINT8_T *)(net_state->input), (inputs_align*sizeof(FLOAT_AI_T)));
    ...
    AiBaseBoardCmodel1AioAddBias(output, biases, batch, m, out_h, out_w, out_h_align, out_w_align, padding_next);
	AiBaseLogLayerData(layer_index, "BiasesOutputCmodel1", (UINT8_T *)(output), (layer_data->output.length));
    AiBaseBoardX86AioActivateArrayCpuCustom(output, m*out_h_align*out_w_align*batch, activation);
	AiBaseLogLayerData(layer_index, "ActivateOutputCmodel1", (UINT8_T *)(output), (layer_data->output.length));

	net_state->output = (FLOAT_AI_T *)AiNetLayerContentGetAddress(&(layer_data->output));

	AiBaseLogLayerData(layer_index, "BiasesCmodel1", (UINT8_T *)biases, layer_data->biases.length);
