...
ERROR_T AiNetLayerYoloForward(AI_NET_STATE *net_state, UINT32_T layer_index) {
	...
	AiBaseBoardCmodel1AioMapCopyCpu(batch, w_align, h_align, c_align, padding, input, out_w, out_h, out_c, 0, output);
...
* comment  V100
*/
INT32_T AiNetLayerYoloEntryIndex(UINT32_T layer_index, INT32_T batch, INT32_T location, INT32_T mblock)
{
	AI_NET_LAYER *net_layer;
	AI_NET_LAYER_DATA *layer_data;
	UINT32_T outputs, w, h;
	INT32_T n, loc;
	INT32_T mblock_index;
	UINT32_T classes;
    ...
    n =   location / (w * h);
    loc = location % (w * h);
    mblock_index = batch*outputs + n*w*h*(4+classes+1) + mblock*w*h + loc;
...
UINT32_T AiNetLayerYoloNumDetections(UINT32_T layer_index, FLOAT_AI_T thresh)
{
    ...
    for (i = 0; i < w*h; i++){
        for(n = 0; n < num; ++n){
            ...
            if(AiBaseFloatCmpGt(output[obj_index], thresh)){
                ++count;
            }
        }
    }
...
ERROR_T AiNetLayerYoloSetLayerContent(AI_NET_LAYER *net_layer, AI_NET_CONFIG *net_config, UINT32_T section_index, AI_NET_NETWORK *net_network)
{
	...
	layer_data->h_align = h;
	layer_data->w_align = ((w + 2 * layer_data->padding + 7) / 8) * 8;
	layer_data->c_align = c;
	layer_data->inputs_align = layer_data->h_align*layer_data->w_align*layer_data->c_align;
	layer_data->out_h_align = out_h;
	layer_data->out_w_align = out_w;
	layer_data->out_c_align = out_c;
	layer_data->outputs_align = outputs;
	layer_data->padding_next = 0;
