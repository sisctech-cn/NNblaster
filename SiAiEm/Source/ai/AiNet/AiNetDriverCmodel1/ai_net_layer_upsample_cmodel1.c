...
extern FLOAT_AI_T g_ai_cmodel1_map_padding_input[AI_NET_NETWORK_BUFFER_LENGTH_MAX];
extern FLOAT_AI_T g_ai_cmodel1_map_padding_output[AI_NET_NETWORK_BUFFER_LENGTH_MAX];
...
ERROR_T AiNetLayerUpsampleForward(AI_NET_STATE *net_state, UINT32_T layer_index) {
    ...
    if(reverse > 0){
		AiBaseBoardCmodel1AioMapCopyCpu(1, out_w_align, out_h_align, out_c_align, padding_next, output, out_w, out_h, out_c, 0, g_ai_cmodel1_map_padding_output);
        AiBaseBoardX86AioUpsampleCpu(g_ai_cmodel1_map_padding_output, out_w, out_h, c, batch, stride, 0, scale, g_ai_cmodel1_map_padding_input);
		AiBaseBoardCmodel1AioMapCopyCpu(1, w, h, c, 0, g_ai_cmodel1_map_padding_input, w_align, h_align, c_align, padding, input);
    }else{
		AiBaseBoardCmodel1AioMapCopyCpu(1, w_align, h_align, c_align, padding, input, w, h, c, 0, g_ai_cmodel1_map_padding_input);
		AiBaseBoardX86AioUpsampleCpu(g_ai_cmodel1_map_padding_input, w, h, c, batch, stride, 1, scale, g_ai_cmodel1_map_padding_output);
		AiBaseBoardCmodel1AioMapCopyCpu(1, out_w, out_h, out_c, 0, g_ai_cmodel1_map_padding_output, out_w_align, out_h_align, out_c_align, padding_next, output);
    }
